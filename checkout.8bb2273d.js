...
function addToSummaryPanel() {
    /*  const summaryPanel = document.createElement('checkout-summary'); */ const product = localStorage.getItem("products") ? JSON.parse(localStorage.getItem("products")) : [];
    const summaryItemsGroup = document.querySelector("#summary-items-flex-group");
    /* const summaryItem = document.createElement('section'); */ /* summaryItem.setAttribute('data-id', `${product.id}`); */ /* summaryItem.classList.add('summary-items-flex'); */ /* console.log(summaryItem.style.display) */ for(let i1 = 0; i1 < product.length; i1++)summaryItemsGroup.innerHTML += `<section class="summary-items-flex"><img
    class="summary-item-flex"
    src="${product[i1].imgSrc}"
    alt="summary-img"
    srcset=""
  />
  
  <section class="summary-item-title-and-price">
    <p class="summary-product-name">${product[i1].productName}</p>
    <p class="summary-product-charge-amount">${product[i1].productPrice * product[i1].productQuantity}</p>
  </section>
  
  <p class="summary-product-count">x${product[i1].productQuantity}</p></section>`;
}
function addToSummaryPanelPrices() {
  ...
  <section class="charges-and-prices">
    ...
    <p class="summary-charges-price" id="summary-grand-total">$${130 + total}</p>
  </section>`;
...
/* figure out whether or not to clear eMoney inputs on cash on delivery selection */ function eMoneyInputPinValidation() {
    let eMoneyInput = document.querySelector("#e-money");
    let eMoneyInputPin = document.querySelector("#e-money-pin");
    if (eMoneyInput.checked) {
...
function formSubmitValidation() {
    ...
    if (eMoneyNumberInput.checked && validateEmail() && validatePhoneNum() && validateEmptyInputs(nameInput) && validateEmptyInputs(addressInput) && validateEmptyInputs(cityInput) && validateEmptyInputs(countryInput) && zipCodeValidation() && eMoneyInputNumberValidation() && eMoneyInputPinValidation() === true) return true;
    else if (cashOndeliveryInput.checked && validateEmail() && validatePhoneNum() && validateEmptyInputs(nameInput) && validateEmptyInputs(addressInput) && validateEmptyInputs(cityInput) && validateEmptyInputs(countryInput) && zipCodeValidation() === true) return true;
    else return false;
}
...
checkoutHamburgerBtn.addEventListener("click", function() {
    let mobileMenu = document.querySelector("#mobile-menu");
    if (mobileMenu.style.display === "none") openCheckoutMobileMenu();
    else closeCheckoutMobileMenu();
});
...
function findCartInfo() {
    let products = getProductsFromStorage();
    /* productTotalPrice(products); */ console.table(products);
    /*  let total = products.reduce((accumulator, products) => {
  return accumulator += products.trueProductPrice;
}, 0);
  console.log(total) */ const total = products.map((products)=>products.trueProductPrice).reduce((prev, curr)=>prev + curr, 0);
    return {
        total: total,
...
form.addEventListener("submit", (e)=>{
    ...
    if (formSubmitValidation() === false) return;
    else {
        openSuccessModal();
        return true;
    }
...
//# sourceMappingURL=checkout.8bb2273d.js.map
