...
async function getDataAsync(productIndex) {
    ...
    if (localStorage.getItem("productData") === null) {
        ...
        let parsedData1 = JSON.parse(localStorage.getItem("productData"));
        inTheBox(parsedData1[productIndex]);
        productFeatureData(parsedData1[productIndex]);
        alternateImagesData(parsedData1[productIndex]);
    ...
    function alternateImagesData(arrEntry) {
        ...
        if (typeof alternateImages != "undefined" && alternateImages != null) alternateImages.innerHTML = `<img id="alternate-img-1" src="${arrEntry.gallery.first.mobile}" alt="gallery-1" srcset="${arrEntry.gallery.first.mobile} 654w,${arrEntry.gallery.first.tablet} 554w, ${arrEntry.gallery.first.desktop} 445w">
    ...
    function inTheBox(arrEntry) {
        ...
        if (typeof inBoxAndCountGroup != "undefined" && inBoxAndCountGroup != null) for(i = 0; i < arrEntry.includes; i++)inBoxAndCountGroup.innerHTML = `<section class="in-box-item-and-count">
...
function getProductInfo(product) {
    ...
    cartItemID++;
    /* productTotalPrice(ProductInfo) */ addToCartList(ProductInfo);
    /* addToSummaryPanel(ProductInfo);
  addToSummaryPanelPrices(ProductInfo); */ saveProductToStorage(ProductInfo);
}
function productTotalPrice(product) {
    for(i = 0; i < product.length; i++)product["trueProductPrice"] = product.productPrice * product.productQuantity;
}
...
} */ /* getDataAsync(3); */ loadCart();
function updateCartInfo() {
    let cartInfo = findCartInfo();
    /* productTotalPrice(cartInfo) */ if (cartInfo.productCount === "" || cartInfo.productCount === null) return;
    else {
        getCartInputsTotal();
...
function addToCartList(product) {
    const cartItem1 = document.createElement("span");
    cartItem1.setAttribute("data-id", `${product.id}`);
    cartItem1.classList.add("cart-item-flex");
    cartItem1.innerHTML = `<section class="cart-item-flex"><img class="cart-item-img" src="${product.imgSrc}" alt="product cart image" srcset="">
  ...
  <form type="post" class=" cart-counter-btn-container">
    ...
    cartList.appendChild(cartItem1);
}
...
function deleteProduct(e) {
    ...
    if (e.target.id === "remove-all-items") {
        for(i = 0; i < cartItems.length; i++)cartItems[i].remove();
        cartItemID = 1;
...
/* check cart count when product is added to cart */ /* document.addEventListener('DOMContentLoaded', function(){
   addToCartBtn.addEventListener('click', purchaseProduct)
}); */ document.addEventListener("click", function(e) {
    if (e.target && e.target.id == "add-to-cart-btn") purchaseProduct();
});
...
document.addEventListener("click", function(e) {
    ...
    } else if (e.target && e.target.id == "increase-btn" && numberBoxValue > 1) decrementBtn.disabled = false;
});
document.addEventListener("click", function(e) {
    let increaseBtns = [];
 ...
 */ incrementShoppingCartInfo(e);
    } else if (e.target && e.target.id == "cart-increase-btn" && numberBoxValue > 1) decrementBtn.disabled = false;
});
function decrementShoppingCartInfo(e) {
    ...
    /* create originalProductQuantity for product object */ let decrementedPrice = convertedCartTotalPrice - parseInt(product[clickedProductIndex].productPrice);
    ...
    Object.assign(product[clickedProductIndex], {
        trueProductPrice: product[clickedProductIndex].productPrice * product[clickedProductIndex].productQuantity,
        productQuantity: decrementedProductCount
    });
...
function incrementShoppingCartInfo(e) {
    ...
    let convertedCartTotalPrice = parseInt(cartTotalPrice.textContent);
    let convertedCartProductCount = parseInt(cartItemCount.textContent);
    /* create originalProductQuantity for product object */ let incrementedPrice = convertedCartTotalPrice + parseInt(product[clickedProductIndex].productPrice);
    ...
    Object.assign(product[clickedProductIndex], {
        trueProductPrice: product[clickedProductIndex].productPrice * product[clickedProductIndex].productQuantity,
        productQuantity: incrementedProductCount
    });
    localStorage.setItem("products", JSON.stringify(product));
    console.log(parseInt(product[clickedProductIndex].productPrice));
    console.table(product);
...
function getCartInputsTotal() {
    const productInputBoxes = document.getElementsByClassName("cart-number-box");
    let total = 0;
    for(let i1 = 0; i1 < productInputBoxes.length; i1++)if (parseInt(productInputBoxes[i1].value)) {
        total += parseInt(productInputBoxes[i1].value);
        console.log(total);
...
//# sourceMappingURL=index.916b3da5.js.map
